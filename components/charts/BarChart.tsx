    ...
    <div className="flex flex-col gap-3" role="img" aria-label="Gráfico de barras">
      {dados.map((item) => (
        <div key={item.rotulo} className="flex items-center gap-3">
          <span className="w-36 shrink-0 truncate text-sm text-foreground-muted">{item.rotulo}</span>
          ...
          <span className="w-10 shrink-0 text-right text-sm font-medium text-foreground">{formatarValor(item.valor)}</span>
        </div>
      ))}
    </div>
  );
